blockr_unloaded <- function() {
  search <- paste0("package:", blockr_pkgs)
  blockr_pkgs[!search %in% search()]
}
...
blockr_attach <- function() {

  to_load <- blockr_unloaded()
...
blockr_attach_message <- function(to_load) {
  ...
  header <- cli::rule(
    left = cli::style_bold("Attaching blockr packages"),
    right = paste0("blockr ", package_version_h("blockr"))
  )
