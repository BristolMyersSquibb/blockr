...
#' @rdname new_block
#' @export
new_transform2_block <- function(data, value = NULL, ...) {
  fields <- list(
  ...
  new_block(
    fields = fields,
    expr = transform2_expr(value = value),
    ...,
    class = c("transform2_block", "transform_block")
  )
}


# Build transform2 expression based on a named character vector
#
# transform2_expr(value = c(a = "2.1", b = "4.5"))
# transform2_expr(value = c(a = '""'))
# transform2_expr(value = character(0))
# transform2_expr(value = NULL)
transform2_expr <- function(value = c(a = "2.1", b = "4.5")) {
  if (is.null(value)) {
    return(quote(dplyr::transform2()))
  }
  ...
  bquote(
    dplyr::transform2(..(exprs)),
    list(exprs = exprs),
    splice = TRUE
  )
}

generate_server.transform2_block <- function(x, in_dat, id, ...) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      o <- observeEvent(
        r_value(), # triggered by module output change
        {
          ...
          attr(blk_updated, "expr") <- transform2_expr(r_value())
          r_blk(blk_updated)
...
update_fields.transform2_block <- function(x, session, data, ...) {
  args <- list(...)
...
#' @rdname new_block
#' @export
transform2_block <- function(data, ...) {
  initialize_block(new_transform2_block(data, ...), data)
}
