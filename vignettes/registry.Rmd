...
If you forget to create a custom method while it is required, you get an error message as a reminder:

```r
Error in `UseMethod()`:
! no applicable method for 'block_output_ptype' applied to an object of class "c('dummy_block', 'data_block', 'block')"
```
...
knitr::include_graphics("figures/cmdk-data.png")
```

```{r cmdk-tranform, echo=FALSE, fig.cap='Scoutbar with data block stack', fig.align = 'center', out.width='50%'}
knitr::include_graphics("figures/cmdk-transform.png")
```
