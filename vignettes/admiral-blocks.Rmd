---
title: "Transform Block for Admiral"
...
Blocks are modular code segments that wrap existing R functions for interactive use in the blockr system. This is a step-by-step guide to create a block using `admiral_derive_param_computed_block` as an example.
...
Now, we want to define the block function in a block creator function, which we call `new_admiral_derive_param_computed_block()`:

```r
new_admiral_derive_param_computed_block <- function() {
  # 0. helper functions
...
Finally, block creation is done with a call to `new_block()`. This is usually left unchanged. In addition, we use a wrapper function, `admiral_derive_param_computed_block`, to initialize and make the block available for blockr.
...
pkgload::load_all(".")
stack <- new_stack(data_block, mutate_block, admiral_derive_param_computed_block)
serve_stack(stack)
```
