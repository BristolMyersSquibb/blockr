...
To create a stack, you call `new_stack()` and feed it all the required blocks.
...
specifying any modules ID, they are automatically handled by `{blockr}`.
...
card(
  create_app_link(
    "NobwRAdghgtgpmAXGKAHVA6ASmANGAYwHsIAXOMpMAdzgCMAnRRASwgGdSoAbbgCgA6YOtyIEA1gyG4ABAzioi7GQF4ZQgBalSqdogD0+kWMkBaIgwDmGSy1IaArnQwsi+2o1PzF7IQEoBCG4WRigGAE8+YwkGAI4uCRkAHlMZCDhqAH1OKAlBCBlCmQATKC5M6PFcQKKZADMWbnIGCtEJQLjA9jgGADc4bITxPhyJPzAAXwBdIA",
    "app",
    header = FALSE
  ),
  full_screen = TRUE
...
In the next example, we'll see how to create an which can dynamically add/remove blocks.
...
currently registered blocks. `add_block_ui()` is a UI helper to show a button as well
